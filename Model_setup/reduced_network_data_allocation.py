...
df_gen = pd.read_csv('../Data_setup/10k_topology_files/eGRID_GO2.csv',header=0)
df_full = pd.read_csv('../Data_setup/10k_topology_files/nodes_to_BA_state.csv',header=0,index_col=0)
...
line_limit_MW_scaling = [0,400,800,1200,1600,2000]

# BA_hurdle_scaling = list(range(0,1050,100))
BA_hurdle_scaling = [-100,-50,0,50,100]

for YY in Years:
    
    df_load = pd.read_csv('../Data_setup/Time_series_data/BA_data/BA_organized_data/BA_load_{}.csv'.format(YY),header=0)
    df_wind = pd.read_csv('../Data_setup/Time_series_data/BA_data/BA_organized_data/BA_wind_{}.csv'.format(YY),header=0,index_col=0)
    df_solar = pd.read_csv('../Data_setup/Time_series_data/BA_data/BA_organized_data/BA_solar_{}.csv'.format(YY),header=0,index_col=0)
    df_solar_wind_cap_EIA = pd.read_csv('../Data_setup/Time_series_data/BA_data/EIA_datasets/EIA860_solar_wind_capacity/EIA_organized_data/BA_solar_wind_capacity_EIA_{}.csv'.format(YY),header=0,index_col=0)
        ...
        for UC in UC_TREATMENTS:
            ...
            for T_p in line_limit_MW_scaling:
                ...
                for BA_hurd in BA_hurdle_scaling:
                    ...
                    FN = 'Selected_nodes/Result_excluded_Nodes_' + str(NN) + '.xlsx'
                    ...
                    for b in BAs:
                        ...
                        corrected = [-x if x<0 else x for x in sample]
                    ...
                    df_summary = pd.read_excel(FN,sheet_name='Summary',header=16)
                    ...
                    for b in BAs:
                        sample = list(df_full.loc[(df_full['NAME']==b) & (df_full['FuelType'] == 'WIND'),'MWMax'])
                    ...
                    df_BA_totals.to_csv('BA_wind_totals.csv')
                    
                    weights = []
                    for i in range(0,len(df_full)):
                        area = df_full.loc[i,'NAME']
                        if area in BAs:
                            if str(df_full.loc[i,'FuelType']) != 'WIND':
                                weights.append(0)
                            else:        
                                X = float(df_BA_totals.loc[df_BA_totals['Name']==area,'Total'])
                    ...
                    for b in BAs:
                        sample = list(df_full.loc[(df_full['NAME']==b) & (df_full['FuelType'] == 'SOLAR'),'MWMax'])
                        # corrected = [0 if math.isnan(x) else x for x in sample]
                        BA_totals.append(sum(sample))
                    ...
                    for i in range(0,len(df_full)):
                        area = df_full.loc[i,'NAME']
                        if area in BAs:
                            if str(df_full.loc[i,'FuelType']) != 'SOLAR':
                                weights.append(0)
                            else:
                                X = float(df_BA_totals.loc[df_BA_totals['Name']==area,'Total'])
                                W = df_full.loc[i,'MWMax']/X
                    ...
                    df_gens = pd.read_csv('../Data_setup/10k_topology_files/eGRID_GO2.csv',header=0)
                    df_gens = df_gens.replace('', np.nan, regex=True)
                    df_gens_heat_rate = pd.read_csv('../Data_setup/10k_topology_files/eGRID_GO2.csv',header=0)
                    old_bus_num =[]
                    new_bus_num = []
                    NB = []
                    ...
                    names = list(df_gens['Name'])
                    fts = list(df_gens['FuelType'])
                    names_hr = list(df_gens_heat_rate['Name'])
                    ...
                    H_min_df.to_csv('Model_inputs/Hydro_min.csv',index=None)
                    ...
                    df_W = pd.read_csv('Model_inputs/nodal_wind.csv',header=0)
                    ...
                    df_S = pd.read_csv('Model_inputs/nodal_solar.csv',header=0)
                    ...
                    df_H = pd.read_csv('Model_inputs/Hydro_max.csv',header=0)
                    ...
                    df = pd.read_csv('Model_inputs/data_genparams.csv',header=0)
                    ...
                    # Natural gas prices
                    NG_price = pd.read_csv('../Data_setup/Time_series_data/Fuel_prices/NG_price/Bias_corrected_NG_prices/Average_NG_prices_BAs_{}.csv'.format(YY), header=0)
                    buses = list(df_selected['bus_i'])
                    for bus in buses:
                    ...
                    # Coal prices
                    Coal_price = pd.read_csv('../Data_setup/Time_series_data/Fuel_prices/Coal_price/Coal_organized_prices/BA_coal_prices_{}.csv'.format(YY), header=0)
            
                    for bus in buses:
                        
                        selected_node_BA = df_full.loc[df_full['Number']==bus,'NAME'].values[0]
                        selected_node_BA = df_BAs.loc[df_BAs['Name']==selected_node_BA]['Abbreviation'].values[0]
                        specific_node_coal_price = Coal_price.loc[:,selected_node_BA].copy()
